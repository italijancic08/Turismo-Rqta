...
Whenever code changes modify behavior, update the documentation in the same commit.
...
When generating documentation with AI:

- Verify every statement against the source code.
- Never invent behavior.
- Never document features that are not implemented.
- Prefer concrete examples over generic explanations.
- Keep examples compilable.
- Preserve GoDoc conventions.