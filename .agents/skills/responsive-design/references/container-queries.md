...
Use Container Queries when a component should adapt to the size of its parent container.
...
Use Media Queries when the entire page layout depends on the viewport.
...
Container Queries complement Media Queries—they do not replace them.
...
- Applying queries to the container instead of its children.
- Nesting unnecessary containers.
- Replacing all Media Queries with Container Queries.
- Missing browser fallbacks.

## Debugging

If a Container Query is not working:

1. Verify `container-type` is set.
2. Ensure the queried element is inside the container.
3. Inspect the container width.
4. Confirm browser support.
5. Test with a fixed width container.

Use DevTools to inspect active container queries.

## Best Practice

Container Queries work particularly well with:
...
- Component inside narrow sidebar.
- Component inside wide layout.
- Nested containers.
- Dynamic resizing.
- Browser compatibility.
- No overflow.

## this Website Examples

Good candidates for Container Queries:
